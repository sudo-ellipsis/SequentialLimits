...
// TODO:
// - optimise tick function for speed
// - rebalance again to have theory milestone power closer and have variable values closer
// - add story chapters that further the chosen narrative and achievements that aren't progression based (secret focused)
// - ensure tertiary renderer works how expected, especially at high values 
// - find a mathematical-concept name to fit with the rest of the game
// - perform extended simulation
// - add another variable? (possibly broken)
// - find optimum permanent upgrade requirements, preferably at stretches
// TODO POST V1:
...
var init = () => {
    ...
    // a1
    {
        ...
        a1 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(1, 0.581*Math.log2(10)))); //0'th upgrade in the list - first cost is 0, other costs are 10 * 2^(3*level), costs currency1
    ...
    // a2
    {
        ...
        a2 = theory.createUpgrade(1, currency, new ExponentialCost(500, Math.log2(10))); //1st upgrade in the list - costs are 5*10^level, costs currency1
    ...
    // b1
    {
        ...
        b1 = theory.createUpgrade(2, currency, new ExponentialCost(1000, 0.705*Math.log2(10))); //2nd upgrade in the list - costs are 100 + 10^level, costs currency1
    ...
    // b2
    {
        ...
        b2 = theory.createUpgrade(3, currency, new ExponentialCost(2500, 1.05*Math.log2(10))); //3rd upgrade in the list - costs are 3*10^(3*level), costs currency1
    ...
    // Permanent Upgrades
    theory.createPublicationUpgrade(0, currency, 1e10 ); //unlock publications at 1e7 currency
    ...
    //milestone 1
    {
        ...
        gamma0.info = Localization.getUpgradeIncCustomExpInfo("\\rho_2", "0.03"); //basically the same but for info
    ...
    //TODO change it to a localisation of decreases lol
    {
        ...
        gamma1.info = Localization.getUpgradeDecCustomInfo("a_3","0.004"); //basically the same but for info
    ...
    //milestone 3
    {
        ...
        gamma2.description = Localization.getUpgradeIncCustomExpDesc("b_1", "0.01"); //set desc as localisation of "increases b1 exponent by 0.02"
        gamma2.info = Localization.getUpgradeIncCustomExpInfo("b_1", "0.01"); //basically the same but for info
    ...
    //milestone 4
    {
        ...
        gamma3.description = Localization.getUpgradeIncCustomExpDesc("b_2", "0.01"); //set desc as localisation of "increases b2 exponent by 0.02""
        gamma3.info = Localization.getUpgradeIncCustomExpInfo("b_2", "0.01"); //basically the same but for info
    ...
    achievement10 = theory.createAchievement(9, "Microscopic", "Get your approximation of e to 10^-20 off true", () => inverseE_Gamma >= BigNumber.From("1e20"));
    achievement11 = theory.createAchievement(10, "Are We There Yet?", "Get your approximation of e to 10^-30 off true", () => inverseE_Gamma >= BigNumber.From("1e30"));
    achievement12 = theory.createAchievement(11, "Subatomic", "Get your approximation of e to 10^-40 off true", () => inverseE_Gamma >= BigNumber.From("1e40"));
    achievement13 = theory.createAchievement(12, "Planck Pettiness", "Get your approximation of e to 10^-50 off true", () => inverseE_Gamma >= BigNumber.From("1e50"));
    achievement14 = theory.createAchievement(13, "Precision Player", "Get your approximation of e to 10^-100 off true", () => inverseE_Gamma >= BigNumber.From("1e100"));
    achievement15 = theory.createAchievement(14, "You Can Stop Anytime", "Get your approximation of e to 10^-250 off true", () => inverseE_Gamma >= BigNumber.From("1e250"));
    achievement16 = theory.createAchievement(15, "You Know You Can Look It Up, Right?", "Get your approximation of e to 10^-500 off true", () => inverseE_Gamma >= BigNumber.From("1e500"));
    ...
    achievement22 = theory.createSecretAchievement(21, "Pattern Fanatic", "Have every variable level the same", "Getting Religous", () => a1.level == a2.level == b1.level == b2.level && a1.level >= 3);
...
//    rho2dot =(geta1(a1.level) * geta2(a2.level) * (BigNumber.TWO-gamma1.level*0.004).pow( - currency3.value.log() )); //calculate rho2dot, accounting for milestones
    rho2dot = a1v > 0 && a2v > 0 ? BigNumber.E.pow(a1v.log() + a2v.log() - (BigNumber.TWO-gamma1.level*0.004).log() * (currency3.value).log() ) : BigNumber.ZERO;
...
var getTertiaryEquation = () => {
    ...
    result += "e - \\gamma = ";
    if (inverseE_Gamma >= 1000){ //display gamma based on whether it's small enough to use E notation
    result += (1/inverseE_Gamma).toNumber().toExponential(4); //use .toNumber() before .toExponential()!
    } else {
        result += (1/inverseE_Gamma).toString(4);
    }
    // render e-approximation and the result, then move to next esgment
    // result += (1/inverseE_Gamma).toString(4);

    // result += ", \\;\\dot{\\rho}_1 = " + rho1dot.toNumber().toExponential(3);
    ...
    // } else{
    //     result +=  (rho2dot).toNumber().toExponential.toString(0) + ", \\;"; 
    // }
    ...
    //     }
        // result += ", \\; np = " + numPublishes.toString(0);   
...
var getPublicationMultiplier = (tau) => 1000*tau.pow(0.12); //publication mult bonus is (tau^0.15)*100
var getPublicationMultiplierFormula = (symbol) => "500 · " + symbol + "^{0.12}"; //text to render for publication mult ext
...
var geta1 = (level) => Utils.getStepwisePowerSum(level, 2, 4.1, 0); //get the value of the variable from a power sum with a level of <level>, a base of 2, a step length of 5 and an initial value of 0 
var geta2 = (level) => BigNumber.TWO.pow(level); //get the value of the variable from a power of 2^level
var getb1 = (level) => Utils.getStepwisePowerSum(level, 3, 6.5, 0); //get the value of the variable from a power sum with a level of <level>, a base of 3, a step length of 2 and an initial value of 0
