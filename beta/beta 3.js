...
var name = "Approximation Exploitation -rc2a"; //display name
...
var inverseE_Gamma = BigNumber.ZERO; //used for the approximation of e
...
var init = () => {
    ...
    // a1
    {
        ...
        a1 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(1, 0.571*Math.log2(10)))); //0'th upgrade in the list - first cost is 0, other costs are 10 * 2^(3*level), costs currency1
    ...
    // b1
    {
        ...
        b1 = theory.createUpgrade(2, currency, new ExponentialCost(200, 0.719*Math.log2(10))); //2nd upgrade in the list - costs are 100 + 10^level, costs currency1
    ...
    // b2
    {
        ...
        b2 = theory.createUpgrade(3, currency, new ExponentialCost(500, 1.047*Math.log2(10))); //3rd upgrade in the list - costs are 3*10^(3*level), costs currency1
    ...
    // Permanent Upgrades
    theory.createPublicationUpgrade(0, currency, 1e7 ); //unlock publications at 1e7 currency
    theory.createBuyAllUpgrade(1, currency, 1e25); //unlock buy all at 1e25 currency
    theory.createAutoBuyerUpgrade(2, currency, 1e35); //unlock autobuyer at 1e35 currency
    ...
    //milestone 1
    {
        ...
        gamma0.description = Localization.getUpgradeIncCustomExpDesc("\\rho_2", "0.04"); //set desc as localisation of "increases rho_2 exponent by 0.01"
        gamma0.info = Localization.getUpgradeIncCustomExpInfo("\\rho_2", "0.04"); //basically the same but for info
    ...
    //TODO change it to a localisation of decreases lol
    {
        ...
        gamma1.description = Localization.getUpgradeIncCustomDesc("a_3","- 0.004"); //set desc as localisation of "increases a3 by - 0.002"
        gamma1.info = Localization.getUpgradeIncCustomInfo("a_3"," - 0.004"); //basically the same but for info
    ...
    achievement10 = theory.createAchievement(9, "Subatomic", "Get your approximation of e to 10^-15 off true", () => inverseE_Gamma >= BigNumber.From("1e15"));
    achievement11 = theory.createAchievement(10, "Are We There Yet?", "Get your approximation of e to 10^-20 off true", () => inverseE_Gamma >= BigNumber.From("1e20"));
    achievement12 = theory.createAchievement(11, "Subatomic", "Get your approximation of e to 10^-25 off true", () => inverseE_Gamma >= BigNumber.From("1e25"));
    achievement13 = theory.createAchievement(12, "Planck Pettiness", "Get your approximation of e to 10^-35 off true", () => inverseE_Gamma >= BigNumber.From("1e35"));
    achievement14 = theory.createAchievement(13, "Precision Player", "Get your approximation of e to 10^-50 off true", () => inverseE_Gamma >= BigNumber.From("1e50"));
    achievement15 = theory.createAchievement(14, "You can stop anytime", "Get your approximation of e to 10^-100 off true", () => inverseE_Gamma >= BigNumber.From("1e100"));
    achievement16 = theory.createAchievement(15, "You know you can look it up, right?", "Get your approximation of e to 10^-200 off true", () => inverseE_Gamma >= BigNumber.From("1e200"));
...
var tick = (elapsedTime, multiplier) => {
    ...
    if (currency3.value < 1000){
        ...
        let logr = (BigNumber.PI * two_pi_rho.log()).log() - two_pi_rho.log();
        inverseE_Gamma = (BigNumber.E.pow(BigNumber.E.pow(logr) - logr) - BigNumber.from(0.5))/BigNumber.E; //xelaroc's method of approximating the approximation that isn't bothered by the floating point limit  
    }
    ...
    rho1dot = (currency2.value.pow(BigNumber.ONE+gamma0.level*BigNumber.from(0.04))).sqrt()*(inverseE_Gamma); //rho1dot is equal to the root of rho2^milestone, over the difference between E and stirling's approximation
...
var getTertiaryEquation = () => {
    ...
    if(inverseE_Gamma <= 10000)
        ...
        let exp = 1+Math.floor(inverseE_Gamma.log10().toNumber()),
            mts = (BigNumber.TEN.pow(exp)/inverseE_Gamma).toString(3 - Math.floor(Math.log10(exp)));
...
var getPublicationMultiplier = (tau) => BigNumber.HUNDRED*tau.pow(0.15); //publication mult bonus is (tau^0.15)*100
var getPublicationMultiplierFormula = (symbol) => "100 \\cdot " + symbol + "^{0.15}"; //text to render for publication mult ext
...
var geta1 = (level) => Utils.getStepwisePowerSum(level, 3.3, 2, 0); //get the value of the variable from a power sum with a level of <level>, a base of 2, a step length of 5 and an initial value of 0 
var geta2 = (level) => BigNumber.TWO.pow(level); //get the value of the variable from a power of 2^level
var getb1 = (level) => Utils.getStepwisePowerSum(level, 6.5, 3, 0); //get the value of the variable from a power sum with a level of <level>, a base of 3, a step length of 2 and an initial value of 0
