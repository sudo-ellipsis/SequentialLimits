...
var two_pi_rho = BigNumber.TWO * BigNumber.PI * currency3.value; //precalculation of values for tick function
...
var init = () => {
    ...
    //TODO change it to a localisation of decreases lol
    {
        ...
        gamma1.description = Localization.getUpgradeIncCustomDesc("a_3","- 0.2"); //set desc as localisation of "increases a3 by - 0.2"
        gamma1.info = Localization.getUpgradeIncCustomInfo("a_3"," - 0.2"); //basically the same but for info
...
var tick = (elapsedTime, multiplier) => { //function that runs every tick
    ...
    rho3dot = bonus*(getb1(b1.level).pow(BigNumber.ONE + gamma2.level*0.05) * getb2(b2.level).pow(1 + gamma3.level*0.05 )); //rho3dot is equal to b1.value * b2.value accounting for exponenents
    ...
    currency2.value += currency2.value + dt * rho2dot; //increase rho2 by rho2dot by dt, accounting for pub bonus
    rho1dot = (currency2.value.pow(BigNumber.ONE+gamma0.level*0.01)).sqrt()*(1/inverseE_Gamma); //rho1dot is equal to the root of rho2^milestone, over the difference between E and stirling's approximation
...
var getPublicationMultiplier = (tau) => tau.pow(0.135); //publication mult bonus is (tau^0.15)/2
var getPublicationMultiplierFormula = (symbol) =>  symbol + "^{0.135}"; //text to render for publication mult ext
...
var geta1 = (level) => Utils.getStepwisePowerSum(level, 8, 6, 0); //get the value of the variable from a power sum with a level of <level>, a base of 2, a step length of 5 and an initial value of 0 
