...
var name = "Approximation Exploitation -rc1"; //display name
...
var init = () => {
    ...
    theory.createBuyAllUpgrade(1, currency, 1e15); //unlock buy all at 1e13 currency1
    theory.createAutoBuyerUpgrade(2, currency, 1e20); //unlock autobuyer at 1e30 currency1
    ...
    //milestone 1
    {
        ...
        gamma0.description = Localization.getUpgradeIncCustomExpDesc("\\rho_2", "0.01"); //set desc as localisation of "increases rho_2 exponent by 0.05"
    ...
    //TODO change it to a localisation of decreases lol
    {
        ...
        gamma1.description = Localization.getUpgradeIncCustomDesc("a_3","- 0.002"); //set desc as localisation of "increases a3 by - 0.2"
    ...
    //milestone 3
    {
        ...
        gamma2.description = Localization.getUpgradeIncCustomExpDesc("b_1", "0.02"); //set desc as localisation of "increases b1 exponent by 0.05"
    ...
    //milestone 4
    {
        ...
        gamma3.description = Localization.getUpgradeIncCustomExpDesc("b_2", "0.02"); //set desc as localisation of "increases b2 exponent by 0.05"
    ...
    achievement2 = theory.createAchievement(2, "Studied Scribbler", "Publish 10 times.", () => 10 == numPublishes);
    achievement2 = theory.createAchievement(3, "Exemplary Essayist", "Publish 25 times.", () => 25 == numPublishes);
    achievement2 = theory.createAchievement(5, "Publication Professional", "Publish 50 times.", () => 50 == numPublishes);
    achievement2 = theory.createSecretAchievement(6, "What's 9 + 10?", "21", "Twenty One", () => a1.level == 19 && a2.level == 21 );
...
var tick = (elapsedTime, multiplier) => {
    ...
    rho3dot = bonus*(getb1(b1.level).pow(BigNumber.ONE + gamma2.level*BigNumber.From(0.02)) * getb2(b2.level).pow(BigNumber.ONE + gamma3.level*BigNumber.From(0.02))); //rho3dot is equal to b1.value * b2.value accounting for exponenents
    currency3.value += rho3dot*dt; //increase currency3.value by rho3dot*dt*bonus
    ...
    rho2dot = bonus*(geta1(a1.level) * geta2(a2.level) * (BigNumber.TWO-gamma1.level*0.002).pow( - currency3.value.log() )); //calculate rho2dot, accounting for milestones
    currency2.value += dt * rho2dot; //increase rho2 by rho2dot by dt, accounting for pub bonus
    ...
    currency.value += dt * bonus * rho1dot; //increase rho1 by rho1dot by dt, accounting for pub bonus
...
var getTertiaryEquation = () => {
    ...
    result += "e - \\gamma = ";
    if (e_gamma <= 0.0001){ //display gamma based on whether it's small enough to use E notation
    ...
    result += (rho2dot).toString(2) + ", \\;"; 
        ...
        result += "\\dot{\\rho}_2 = " + (rho2dot).toString(1) + ", \\;"; 
    } if (1000 < rho2dot < 10000 ){   
        result += "\\dot{\\rho}_2 = " + (rho2dot).toString(0) + ", \\;"; 
    } else{
        result += "\\dot{\\rho}_2 = " + (rho2dot).toNumber.toExponential.toString(2) + ", \\;"; 
    }
    ...
    if (rho3dot<100){
        result += (rho3dot).toString(2) + ", \\;"; 
            ...
            result += "\\dot{\\rho_3} = " + (rho3dot).toString(2) + " \\;";
        } if (1000 < rho3dot < 10000 ){   
            result += "\\dot{\\rho_3} = " + (rho3dot).toString(1) + " \\;";
        } else{
            result += "\\dot{\\rho}_2 = " + (rho3dot).toNumber.toExponential.toString(2) + ", \\;"; 
        }
...
var geta1 = (level) => Utils.getStepwisePowerSum(level, 2, 10, 0); //get the value of the variable from a power sum with a level of <level>, a base of 2, a step length of 5 and an initial value of 0 
var geta2 = (level) => BigNumber.TWO.pow(level); //get the value of the variable from a power of 2^level
var getb1 = (level) => Utils.getStepwisePowerSum(level, 10, 2, 0); //get the value of the variable from a power sum with a level of <level>, a base of 3, a step length of 2 and an initial value of 0
